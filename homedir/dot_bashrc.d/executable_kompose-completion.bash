...
__kompose_handle_go_custom_completion()
{
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __kompose_debug "File filtering command: $filteringCmd"
        $filteringCmd
        ...
        if [ -n "$subdir" ]; then
            __kompose_debug "Listing directories in $subdir"
            __kompose_handle_subdirs_in_dir_flag "$subdir"
        else
            __kompose_debug "Listing directories in ."
            _filedir -d
        fi
    else
...
_kompose_convert()
{
    ...
    flags+=("--push-image-registry=")
    two_word_flags+=("--push-image-registry")
    local_nonpersistent_flags+=("--push-image-registry")
    local_nonpersistent_flags+=("--push-image-registry=")
    flags+=("--pvc-request-size=")
    two_word_flags+=("--pvc-request-size")
    local_nonpersistent_flags+=("--pvc-request-size")
    local_nonpersistent_flags+=("--pvc-request-size=")
    flags+=("--replicas=")
    two_word_flags+=("--replicas")
    local_nonpersistent_flags+=("--replicas")
    local_nonpersistent_flags+=("--replicas=")
    flags+=("--secrets-as-files")
    local_nonpersistent_flags+=("--secrets-as-files")
    flags+=("--service-group-mode=")
    two_word_flags+=("--service-group-mode")
    local_nonpersistent_flags+=("--service-group-mode")
    local_nonpersistent_flags+=("--service-group-mode=")
    flags+=("--service-group-name=")
    two_word_flags+=("--service-group-name")
    local_nonpersistent_flags+=("--service-group-name")
    local_nonpersistent_flags+=("--service-group-name=")
    flags+=("--stdout")
